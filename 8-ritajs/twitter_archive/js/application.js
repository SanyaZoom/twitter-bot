...
//
  var templates = {
    ...
    user_nav: Hogan.compile('<a href="#" class="icon-sprite icon-user dropdown-toggle" data-toggle="dropdown"></a><ul class="dropdown-menu"><li><a href="#user-info" data-toggle="modal"><i class="icon-user"></i>Подробнее об учётной записи</a></li><li class="divider"></li><li><a href="https://twitter.com/{{screen_name}}" target="_blank"><i class="icon-share-alt"></i>Показать профиль в Твиттере</a></li></ul>'),
    modal_header: Hogan.compile('<h3>Детали учётной записи <span class="download-date muted">по состоянию на {{created_at_relative}}</span></h3>'),
    ...
    modal_payload_details: Hogan.compile('<p><span class="footer-label muted">твиты: </span>{{tweets}}</p>'),
    modal_account_details: Hogan.compile('<p>#{{id}} <span class="footer-label muted">ID пользователя</span></p><p class="truncated">{{created_at_relative}} <span class="footer-label muted">Присоединился</span></p>'),
    ...
    singular_tweet_count: Hogan.compile('{{count}} твит'),
    plural_tweet_count: Hogan.compile('Твитов: {{count}}')
  };
//
  ...
  exports.init = function () {
    ...
    .bind('invalidSearch', function (e) {
      e.preventDefault();
      $(this)
        .attr('data-original-title', 'Ваш запрос должен содержать не менее двух символов.')
        .tooltip('show');
    });
  ...
  (function(exports) {
    exports.base = {
      init: function () {
        $('.navbar-search .search-query').attr('placeholder', 'Поиск по всем твитам');
  ...
  (function(exports) {
    var Tweets = function () {
      this.str_singular       = 'Твитнуть';
      this.str_plural         = 'Твиты';
  ...
  (function (exports){
    ...
    exports.localizeStrings = function() {
      ...
      document.title = "Ваш архив Твиттера";
      $('#footer-text').append("Это оффлайн-архив ваших твитов. Используйте месяцы сверху для навигации по архиву.");
      var compose_new_tweet = "Новый твит";
